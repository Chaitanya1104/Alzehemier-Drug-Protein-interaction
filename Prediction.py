...
def predictor(models_path, data_path):
    ...
    # data_df.to_csv('output/molecules.csv', index=False)  # Save the input data
    # Calculate fingerprints
    df_fingerprints = to_fingerprint(data_df, 'morgan2_c')
    df_fingerprints.to_csv('./Prediction/fingerprints.csv', index=False)  # Save fingerprints
    df_fingerprints = decode_fingerprints(df_fingerprints)
    print(f'Fingerprints shape: {df_fingerprints.shape}')
    # df_fingerprints.to_csv('./Prediction/fingerprints.csv', index=False)  # Save fingerprints
